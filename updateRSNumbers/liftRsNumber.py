...
if __name__ == '__main__':
    ...
    for ln in myopen('/scratch/jclab/sathvik/sathvik/SNP_LDs/data/updateRSNumbers/SNPHistory.bcp.gz'):
	fd = ln.strip().split('\t')
	if ln.find('re-activ') < 0:
	    RS_HISTORY.add(fd[0])

    # record rs number merge history
    for ln in myopen('/scratch/jclab/sathvik/sathvik/SNP_LDs/data/updateRSNumbers/RsMergeArch.bcp.gz'):
	fd = ln.strip().split('\t')
	h, l = fd[0], fd[1]
	c = fd[6]
	# print 'c=', c
